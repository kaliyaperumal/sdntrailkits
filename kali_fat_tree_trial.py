...
class dcFatTreeTopo( Topo ):
	...
	def __init__(self, k=2, **opts): 
		...
		for i in irange(0, k-1):
			...
			cmd = "ovs-vsctl set Bridge " +'c%s%s' % (i+1, 0)+ " stp_enable=true"
			print cmd
			stplist.append( cmd )
		...
		for i in irange(1, k):
			...
			cmd = "ovs-vsctl set Bridge " +'a%s%s' % (i, 1)+ " stp_enable=true"
			stplist.append( cmd )
			aggSwitch2 = self.addSwitch('a%s%s' % (i, 3))
			cmd = "ovs-vsctl set Bridge " +'a%s%s' % (i, 3)+ " stp_enable=true"
			stplist.append( cmd )
			aggList.append(aggSwitch1)
			aggList.append(aggSwitch2)
			torSwitch1 = self.addSwitch('t%s%s' % (i, 2))
			cmd = "ovs-vsctl set Bridge " +'t%s%s' % (i, 2)+ " stp_enable=true"
			stplist.append( cmd )
			torSwitch2 = self.addSwitch('t%s%s' % (i, 4))
			cmd = "ovs-vsctl set Bridge " +'t%s%s' % (i, 4)+ " stp_enable=true"
			stplist.append( cmd )
			...
			self.addLink(torSwitch1, aggSwitch1, cls=TCLink, bw=5, delay='10ms', loss=0, max_queue_size=500, use_htb=True)
			self.addLink(torSwitch1, aggSwitch2, cls=TCLink, bw=5, delay='10ms', loss=0, max_queue_size=500, use_htb=True)
			"connection of the the right tor switch to aggregation switches"
			self.addLink(torSwitch2, aggSwitch1, cls=TCLink, bw=5, delay='10ms', loss=0, max_queue_size=500, use_htb=True)
			self.addLink(torSwitch2, aggSwitch2, cls=TCLink, bw=5, delay='10ms', loss=0, max_queue_size=500, use_htb=True)
			...
			for r in irange(0, (k*2)-1): #this is to go through the agg switches
				if r % 2 == 0: #if agg switch is even then connect to first half
					for j in irange(0, ((k/2)-1)): #this is to go through the core switches
						self.addLink(aggList[r], coreList[j], cls=TCLink, bw=10, delay='1ms', loss=0, max_queue_size=1000, use_htb=True) 
				else:
